...
class SyntheticSequenceDataset(torch.utils.data.Dataset):
    def __init__(self, vocab_size: int = 500, seq_len: int = 32, num_samples: int = 4000, seed: int = 42):
        super().__init__()
...
def run_capacity_benchmark():
    print("=" * 60)
    print("   DUOBIT Phase 2 Capacity Scaling & Parity Benchmark")
    print("=" * 60)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Device: {device}")

    train_dataset = SyntheticSequenceDataset(vocab_size=500, seq_len=32, num_samples=4000, seed=42)
    ...
    experiments = {
        ...
        "DUOBIT Phase 2 Scaled 2.0x (d=512)": {
            "use_duobit": True,
            "config": DuobitConfig(
                vocab_size=500, d_model=512, n_layers=4, n_heads=8, d_ff=1408, max_seq_len=32, group_size=128,
                duobit_lr=8e-3, use_swiglu=True, scale_ema_alpha=0.01, scale_update_freq=50, transition_temperature=0.8, min_transitions_per_group=0
            )
        },
        "DUOBIT Phase 2 Scaled 3.0x (d=640)": {
            "use_duobit": True,
            "config": DuobitConfig(
                vocab_size=500, d_model=640, n_layers=4, n_heads=10, d_ff=1792, max_seq_len=32, group_size=128,
                duobit_lr=8e-3, use_swiglu=True, scale_ema_alpha=0.01, scale_update_freq=50, transition_temperature=0.8, min_transitions_per_group=0
            )
        }
    ...
    for name, item in experiments.items():
        ...
        print(f"  -> Final Val Loss: {val_loss:.4f} | Val PPL: {val_ppl:.2f} | TransRate: {avg_trans_rate*100:.2f}% | Time: {elapsed:.1f}s")
        print(f"     Gen Tokens: {gen_tokens[:12]}")

    # Plot
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

    for name, r in results.items():
        ax1.plot(r["steps"], r["losses"], label=f"{name} (Loss: {r['val_loss']:.2f}, {r['memory_mb']:.1f}MB)")
        if "FP32" not in name:
            ax2.plot(r["steps"], [tr * 100 for tr in r["trans_rates"]], label=name)

    ax1.set_title("Validation Loss Progression (1500 steps)")
    ax1.set_xlabel("Step")
    ax1.set_ylabel("Loss")
    ax1.grid(True, alpha=0.3)
    ax1.legend(fontsize=8)

    ax2.set_title("DUOBIT Transition Rate (%)")
    ax2.set_xlabel("Step")
    ...
    plt.tight_layout()
    chart_path = "/root/.gemini/antigravity-ide/brain/51c7ddfc-7552-429f-8f8c-c893e80a2451/duobit_phase2_capacity_scaling.png"
    plt.savefig(chart_path, dpi=150)
    plt.close()

    summary_path = "/kaggle/working/workspace/duobit_phase2_capacity_summary.json"
    with open(summary_path, "w") as f:
        json.dump(results, f, indent=2)

    print(f"\nSaved capacity benchmark plot to {chart_path}")
    print(f"Saved capacity summary to {summary_path}")
