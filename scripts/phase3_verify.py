#!/usr/bin/env python3
"""Small-scale verification of DUOBIT-EST v3 claims.

Runs on CPU in a few minutes:
  1. Codebook reconstruction MSE (binary / ternary / 2-bit, max-abs vs MSE scales)
  2. Quadratic bowl: PEFA vs frozen-code ECO vs STE-with-master-weights
  3. Tiny decoder LM on a synthetic 3-token grammar
  4. Ablations (no PEFA, no ECO, W2A8, ternary, binary)
  5. Packed inference-memory accounting

Writes JSON + PNG figures for the paper.
"""
...
from duobit.quantization.memory import inference_memory_report
from duobit.training.trainer import Trainer

OUT_DIR = os.environ.get("DUOBIT_OUT", os.path.join(ROOT, "public"))
...
# ---------------------------------------------------------------------------
# Experiment 2 — quadratic bowl / linear regression
# ---------------------------------------------------------------------------
def experiment_quadratic(steps: int = 80, seed: int = 0) -> Dict[str, Any]:
    ...
    def run(name: str, use_duobit: bool, use_pefa: bool, enable_ec: bool) -> Dict[str, Any]:
        torch.manual_seed(seed)
        if use_duobit:
            ...
            opt = DuobitAdam(
                ...
                duobit_lr=0.08,
                use_pefa=use_pefa,
                enable_error_compensation=enable_ec,
    ...
    runs = [
        run("FP32 Adam", False, False, False),
        run("DUOBIT v1 (ECO, no PEFA)", True, False, True),
        run("DUOBIT v3 (PEFA + ECO)", True, True, True),
        run("DUOBIT PEFA, no ECO", True, True, False),
        run("DUOBIT no PEFA, no ECO", True, False, False),
    ]
    return {"steps": list(range(1, steps + 1)), "runs": runs}


# ---------------------------------------------------------------------------
# Experiment 3 — tiny LM on synthetic grammar
# ---------------------------------------------------------------------------
...
def grammar_token_accuracy(tokens: List[int], vocab: int) -> float:
    """Fraction of positions i>=1 that obey the triple grammar."""
    if len(tokens) < 3:
        return 0.0
    ok = 0
    n = 0
...
def train_one(
    ...
    mem = inference_memory_report(model)
    logs = res["train_logs"]
    return {
        "name": name,
        "use_duobit": use_duobit,
        "n_levels": cfg.resolved_n_levels() if use_duobit else 0,
        "val_loss": val_loss,
        "val_ppl": val_ppl,
        "elapsed_sec": elapsed,
        ...
        "memory": mem,
        "n_params": int(mem["n_params"]),
        "memory_mb": mem["total_mb"],
        "avg_bits": mem["avg_bits_per_param"],
        "linear_bits": mem["linear_bits_per_weight"],
...
def experiment_lm(steps: int = 60, device: str = "cpu") -> Dict[str, Any]:
    vocab, seq = 96, 16
    ...
    specs = [
        ("FP32 baseline", make_tiny_config(), False),
        ("DUOBIT v3 2-bit PEFA", make_tiny_config(use_pefa=True, n_levels=4), True),
        ("DUOBIT v1 ECO only", make_tiny_config(use_pefa=False, n_levels=4), True),
        ("DUOBIT PEFA, no ECO", make_tiny_config(use_pefa=True, enable_error_compensation=False, n_levels=4), True),
        ("DUOBIT ternary PEFA", make_tiny_config(use_pefa=True, n_levels=3), True),
        ("DUOBIT binary PEFA", make_tiny_config(use_pefa=True, n_levels=2), True),
        ("DUOBIT W2A8 PEFA", make_tiny_config(use_pefa=True, n_levels=4, activation_bits=8), True),
    ]
    runs = []
    for name, cfg, use_d in specs:
        ...
        print(
            ...
            f"trans={r['mean_trans']*100:.3f}% bits={r['avg_bits']:.2f} "
            f"mem={r['memory_mb']:.3f}MB acc={r['grammar_acc']:.2f}"
        )
    return {"steps": steps, "prompt": prompt, "runs": runs}


def experiment_width(steps: int = 40, device: str = "cpu") -> Dict[str, Any]:
    vocab, seq = 96, 16
    ...
    prompt = [10, 31, 36, 12]
    runs = []
    for d, name, use_d, nlev in [
        (64, "FP32 d=64", False, 4),
        (64, "DUOBIT d=64", True, 4),
        (96, "DUOBIT d=96 (width+)", True, 4),
    ]:
        cfg = make_tiny_config(d_model=d, d_ff=d * 2, n_heads=2 if d == 64 else 3, n_levels=nlev)
        print(f"\n===== width {name} =====")
        runs.append(train_one(name, cfg, use_d, train_loader, val_loader, steps, device, prompt))
    return {"runs": runs}
...
def plot_all(mse, quad, lm, width):
    ...
    style_ax(ax)
    palette = [INK, MUTED, SLATE, "#7A8B99", RULE]
    ...
    style_ax(ax)
    for i, run in enumerate(quad["runs"]):
        if run["name"].startswith("FP32"):
            continue
        ax.plot(quad["steps"], [100 * t for t in run["trans_rates"]], color=palette[i % len(palette)], lw=1.6, label=run["name"])
    ax.set_xlabel("Step")
    ...
    style_ax(ax)
    colors = [INK, SLATE, MUTED, "#7A8B99", "#4E5D4A", "#8C5E4A", RULE]
    ...
    for i, run in enumerate(lm["runs"]):
        if not run["use_duobit"]:
            continue
        ax.plot(run["steps"], [100 * t for t in run["trans_rates"]], color=colors[i % len(colors)], lw=1.6, label=run["name"])
    ax.set_xlabel("Step")
    ax.set_ylabel("Transition rate (%)")
    ax.set_title("LM code-flip rate under PEFA vs ECO-only")
    ax.legend(frameon=False, fontsize=7.5)
    fig.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, "fig_lm_transitions.png"), dpi=160)
    plt.close(fig)

    # memory vs loss scatter
    fig, ax = plt.subplots(figsize=(7.0, 4.2))
    style_ax(ax)
    for i, run in enumerate(lm["runs"]):
        ax.scatter(run["memory_mb"], run["val_loss"], color=colors[i % len(colors)], s=42, zorder=3)
        ax.annotate(run["name"], (run["memory_mb"], run["val_loss"]), textcoords="offset points", xytext=(6, 4), fontsize=7, color=INK)
    ax.set_xlabel("Packed inference memory (MB)")
    ax.set_ylabel("Validation loss")
    ax.set_title("Accuracy–memory frontier (tiny LM)")
    fig.tight_layout()
    ...
    for i, run in enumerate(width["runs"]):
        ax.plot(run["steps"], run["losses"], color=colors[i % len(colors)], lw=1.8, label=f"{run['name']} ({run['memory_mb']:.3f} MB)")
    ax.set_xlabel("Step")
    ax.set_ylabel("Training loss")
...
def main():
    ...
    for r in quad["runs"]:
        print(f"  {r['name']:32s} loss={r['final_loss']:.5f} mean_trans={r['mean_trans']*100:.3f}%")
    ...
    payload = {
        ...
        "historical": {
            "phase1_500step": {
                "note": "Prior 5.25M-param run on the same grammar (d=256, 6 layers, 500 steps).",
                "fp32_val_loss": 0.1326,
                "duobit_val_loss": 4.5164,
                "fp32_mem_mb": 20.01,
                "duobit_mem_mb": 2.27,
                "duobit_bits": 3.637,
            },
            "phase1_capacity": {
                "fp32_d256_gen_match": 1.0,
                "duobit_d512_gen_match": 1.0,
                "duobit_d512_mem_mb": 6.94,
                "fp32_d256_mem_mb": 20.01,
            },
            "best_duobit_lr": 0.008,
        },
    }
    out_json = os.path.join(DATA_DIR, "phase3_results.json")
